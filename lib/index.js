...
export class Basura {
  ...
  generate_Buffer(depth = 0) {
    ...
    if (depth > this.opts.depth) {
      ...
      buf = this._randBytes(len, 'Buffer');
    }
    if (this.opts.output) {
  ...
  generate_WeakSet(depth = 0) {
    const vals = [];
    if (depth <= this.opts.depth) {
      const len = this._upto(this.opts.arrayLength, 'weakSetSize');
      for (let i = 0; i < len; i++) {
        const gen = this._pick(this.validWeak, 'weakSetClass');
        const typ = this.types[gen];
        vals.push(typ.call(this, depth + 1));
      }
    }
  ...
  //   const entries = [];
  //   if (depth <= this.opts.depth) {
  //     const len = this._upto(this.opts.arrayLength, 'weakMapSize');
  //     for (let i = 0; i < len; i++) {
  //       const kc = this.types[this._pick(this.validWeak, 'weakMapKeyClass')];
  //       entries.push([kc.call(this, depth + 1), this.generate(depth + 1)]);
  //     }
  //   }
  //   const ret = new WeakMap(entries);
  //   this.weakMembers.set(ret, entries.map(([k]) => k));
  //   return ret;
  // }
