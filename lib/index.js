...
import {Random, randBytes} from './random.js';
import {
...
export class Basura {
  ...
  constructor(opts = {}) {
    ...
    this.typeNames[Random.FREQS] = typeEntries.map(([_nm, fn]) => fn.freq ?? 1);
  }

  _randWeak(depth = 0, reason = 'unspecified') {
  ...
  generate_number(depth = 0) {
    ...
    while (isNaN(n) || !isFinite(n)) {
      n = this.rand.bytes(8, 'number').readDoubleBE(0);
    }
    return n;
  }
  ...
  generate_Buffer(depth = 0) {
    ...
    if (depth > this.opts.depth) {
      ...
      buf = this.rand.bytes(len, 'Buffer');
    }
    if (this.opts.output) {
  ...
  generate_string(depth = 0, reason = 'string') {
    const script = this.rand.pick(this.opts.scripts, `script,${reason}`);
  ...
  generate_RegExp(depth = 0) {
    ...
    do {
      try {
        ...
        if (this.opts.output) {
          setInspect(n, `new RegExp('${n.source}', '${n.flags}')`);
        }
      } catch (ignored) {
  ...
  generate_TypedArray(depth = 0) {
    ...
    const buf = this.rand.bytes(sz * len, nm);
  ...
  generate_WeakSet(depth = 0) {
    ...
    if (depth <= this.opts.depth) {
      ...
      for (let i = 0; i < len; i++) {
        vals.push(this._randWeak(depth, 'weakSetClass'));
      }
    }
  ...
  generate_WeakMap(depth = 0) {
    ...
    if (depth <= this.opts.depth) {
      ...
      for (let i = 0; i < len; i++) {
        entries.push([
          this._randWeak(depth, 'weakMapKeyClass'),
  ...
  generate_WeakRef(depth = 0) {
    ...
    const o = this._randWeak(depth - 1, 'weakRef');
  ...
  static annotations = [
    ['Array', weak],
    ['Boolean', boxed, cborUnsafe, jsonUnsafe, weak],
    ['Buffer', jsonUnsafe, weak],
    ['Date', jsonUnsafe, weak],
    ['Error', cborUnsafe, jsonUnsafe, weak],
    ['Generator', freq(0.1), cborUnsafe, jsonUnsafe],
    ['Map', jsonUnsafe, weak],
    ['Number', boxed, cborUnsafe, jsonUnsafe, weak],
    ['Object', weak],
    ['Promise', cborUnsafe, jsonUnsafe, weak],
    ['Proxy', cborUnsafe, jsonUnsafe, weak],
    ['RegExp', jsonUnsafe, weak],
    ['Set', jsonUnsafe, weak],
    ['String', boxed, cborUnsafe, jsonUnsafe, weak],
    ...
    ['bigint', jsonUnsafe],
    ['function', cborUnsafe, jsonUnsafe],
